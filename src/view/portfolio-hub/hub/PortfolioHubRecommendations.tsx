...
import { PortfolioCategory, PortfolioSubCategory } from '../../../service/recentActivityService/types';
...
const PortfolioHubRecommendations: React.FC<PortfolioHubRecommendationsProps> = ({ className, onCategoryChange }) => {
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      {recommendations.length > 0 && (
        <div className="text-xs text-gray-500 mt-4">
          {t('hubRecommendations.totalRecommendations', { count: recommendations.length })}
        </div>
      )}
    </div>
  );
