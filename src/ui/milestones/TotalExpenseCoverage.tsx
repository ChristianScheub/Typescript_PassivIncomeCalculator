        ...
        <div className="flex justify-between text-sm">
          <div className="space-y-2">
            <p className="font-semibold text-emerald-600 dark:text-emerald-400">
              {t("forecast.milestones.totalExpenseCoverage.percentCovered", {
                percent: percentage.toFixed(1),
              })}
            </p>

            <p className="text-gray-700 dark:text-gray-300 font-medium">
              {t("forecast.milestones.totalExpenseCoverage.passiveIncome", {
                income: formatService.formatCurrency(monthlyPassiveIncome),
              })}
            </p>

            <p className="text-gray-500 dark:text-gray-400">
              {t(
                "forecast.milestones.totalExpenseCoverage.totalMonthlyExpenses",
                { amount: formatService.formatCurrency(totalMonthlyExpenses) }
              )}
            </p>

            <div className="text-xs text-gray-400 dark:text-gray-500 space-y-1">
              <p className="font-medium">
                {t("forecast.milestones.totalExpenseCoverage.breakdownTitle")}
              </p>
              <p className="ml-2">
                {t("forecast.milestones.totalExpenseCoverage.regularExpenses", {
                  expenses: formatService.formatCurrency(monthlyExpenses),
                })}
              </p>
              <p className="ml-2">
                {t(
                  "forecast.milestones.totalExpenseCoverage.liabilityPayments",
                  {
                    debts: formatService.formatCurrency(
                      monthlyLiabilityPayments
                    ),
                  }
                )}
          ...
          {(() => {
            ...
            return (
              <div className="milestone milestone-achieved">
                {t(
                  `forecast.milestones.totalExpenseCoverage.${getMilestoneKey(
                    highestMilestone
                  )}`
                )}
              </div>
