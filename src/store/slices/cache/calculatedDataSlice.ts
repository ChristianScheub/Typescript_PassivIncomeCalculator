...
export const calculatePortfolioHistory = createAsyncThunk(
  ...
  async (
    ...
    Logger.infoRedux(`CalculatedData: Starting portfolio history calculation for ${timeRange}`);
    ...
    switch (timeRange) {
      ...
      case 'ALL':
      default:
        Logger.infoRedux(`CalculatedData: Portfolio history calculation completed for ${timeRange} - ${fullHistory.length} points`);
        return {
          timeRange,
          data: fullHistory,
    ...
    Logger.infoRedux(`CalculatedData: Portfolio history calculation completed for ${timeRange} - ${filteredHistory.length} points`);
    return {
      timeRange,
...
export const calculateAssetFocusData = createAsyncThunk(
  ...
  async (_, { getState }) => {
    ...
    Logger.infoRedux('CalculatedData: Starting asset focus calculation');
    
    const result = assetFocusService.calculateAssetFocusData(assets, assetDefinitions);
    
    Logger.infoRedux('CalculatedData: Asset focus calculation completed successfully');
    return {
      ...result,
      lastCalculated: new Date().toISOString(),
      inputHash
...
const calculatedDataSlice = createSlice({
  ...
  reducers: {
    ...
    clearAllCache: (state) => {
      ...
      StateHydrationService.removeStorageValue('passiveIncomeCalculator');
