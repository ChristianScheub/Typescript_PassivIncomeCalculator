...
export class StockPriceUpdater {
  ...
  static async updateStockHistoricalDataWithPeriod(definitions: AssetDefinition[], period: TimeRangePeriod): Promise<AssetDefinition[]> {
    ...
    if (stockDefinitionsToUpdate.length === 0) {
      Logger.infoService('No stock definitions found to update historical data');
      return [];
    }
    ...
    try {
      ...
      for (const definition of stockDefinitionsToUpdate) {
        try {
          ...
          const historicalData = await this.fetchHistoricalDataWithPeriod(stockAPI, definition.ticker!, period);
          ...
          if (historicalData?.data && historicalData.data.length > 0) {
            ...
            const finalDefinition = {
              ...updatedDefinition,
              priceHistory: cleanupOldPriceHistory(updatedDefinition.priceHistory, 1000) // Keep more entries for longer periods
            };

            updatedDefinitions.push(finalDefinition);
            Logger.infoService(`Updated historical data (${period}) for ${definition.ticker} (${definition.fullName}): ${historicalData.data.length} entries`);
          }
        } catch (error) {
          Logger.error(`Failed to update historical data for ${definition.ticker}: ${error}`);
        }
      ...
      Logger.infoService(`Successfully updated historical data (${period}) for ${updatedDefinitions.length} stock definitions`);
      return updatedDefinitions;
    } catch (error) {
      Logger.error(`Stock API service not available for historical data: ${error}`);
      throw new Error('Stock API service not available. Please check your API configuration in Settings.');
  ...
  private static async fetchHistoricalDataWithPeriod(
    ...
    Logger.infoService(`Fetching ${period} (${days} days) of historical data for ${ticker}`);
