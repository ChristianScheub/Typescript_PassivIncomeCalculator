...
const AssetFocusDashboardContainer: React.FC = () => {
  ...
  const [selectedAssetDefinition, setSelectedAssetDefinition] =
  ...
  const reduxState = useAppSelector((state: RootState) => ({
    ...
    portfolioHistory:
      state.transactions.cache?.history?.[
        state.config.dashboard.assetFocus.timeRange
      ],
  ...
  const portfolioHistoryViewData = usePortfolioHistoryView(
  ...
  const assetFocusData = {
    data: reduxState.assetFocusCache
      ? {
          ...
          assetsWithValues: Array.isArray(
  ...
  const portfolioHistoryData = {
    data: reduxState.portfolioHistory?.data || [],
  };
  ...
  useEffect(() => {
    ...
    const hasData =
      liabilitiesLength > 0 || expensesLength > 0 || incomeLength > 0;
    ...
    if (hasData && !hasValidFinancialSummary) {
      Logger.info(
        "AssetDashboardContainer: Financial summary missing or all zero, triggering calculation"
      );
      Logger.info(
        `AssetDashboardContainer: Data available - income: ${incomeLength}, expenses: ${expensesLength}, liabilities: ${liabilitiesLength}`
      );
  ...
  useEffect(() => {
    ...
    if (hasAssets && !hasValidAssetFocusData) {
      Logger.info(
        "AssetDashboardContainer: Asset focus data missing or empty, triggering calculation"
      );
      Logger.info(
        `AssetDashboardContainer: Data available - transactions: ${assetsLength}, assetDefinitions: ${assetDefinitionsLength}`
      );
      (dispatch as ThunkDispatch<RootState, unknown, AnyAction>)(
        calculateAssetFocusData()
      );
  ...
  const enhancedPortfolioHistory = useMemo(() => {
    ...
    Logger.info(
      `AssetFocusDashboardContainer enhancedPortfolioHistory: timeRange=${reduxState.assetFocus.timeRange}, baseData=${safeBaseData.length}, newSystemData=${newSystemData.length}, intradayData=${intradayData.length}`
    );
    ...
    if (reduxState.assetFocus.timeRange === "1T") {
      ...
      if (reduxState.assetFocus.timeRange === "1T" && intradayData.length > 0) {
        ...
        if (intradayData.length > 0) {
          ...
          Logger.info(
            `1T: Showing last 24h of available intraday data from ${mostRecentTimestamp.toISOString()}: ${
              filteredIntradayData.length
            } points from ${intradayData.length} total`
          );
        }
        ...
        Logger.info(
          `Sample intraday history points: ${JSON.stringify(
            intradayHistoryPoints.slice(0, 3)
          )}`
        );

        // Combine base data with filtered intraday and sort by timestamp
        const combined = [...safeBaseData, ...intradayHistoryPoints].sort(
          (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
        );

        Logger.info(
          `Combined portfolio history: ${combined.length} total points (${safeBaseData.length} daily + ${intradayHistoryPoints.length} intraday)`
        );
        return combined;
      }
      ...
      if (newSystemData.length > 0) {
        Logger.info(
          `Using new system data for ${reduxState.assetFocus.timeRange}: ${newSystemData.length} points`
        );
      ...
      Logger.info(
        `Using base data for ${reduxState.assetFocus.timeRange}: ${safeBaseData.length} points`
      );
      return safeBaseData;
    }
    ...
    if (reduxState.assetFocus.timeRange === "5D" && newSystemData.length > 0) {
      Logger.info(
        `Using combined 5D history from usePortfolioHistoryView: ${newSystemData.length} points`
      );
      // Sortiere alle Punkte nach Datum/Zeit
      const sorted = [...newSystemData].sort(
        (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
      );
    ...
    if (
      (reduxState.assetFocus.timeRange === "1M" ||
        reduxState.assetFocus.timeRange === "3M" ||
        reduxState.assetFocus.timeRange === "6M" ||
        reduxState.assetFocus.timeRange === "1Y" ||
        reduxState.assetFocus.timeRange === "Max") &&
      newSystemData.length > 0
    ) {
      Logger.info(
        `Using new portfolio history system for ${reduxState.assetFocus.timeRange}: ${newSystemData.length} points`
      );
      return newSystemData.map((point) => ({
        date: point.date,
        totalValue: point.totalValue,
    ...
    Logger.info(
      `Using regular daily data for timeRange=${reduxState.assetFocus.timeRange}: ${safeBaseData.length} points`
    );
    return safeBaseData;
  }, [
  ...
  // Get data from hooks
  const assetFocusDataResult = assetFocusData.data || {
    assetsWithValues: [],
    portfolioSummary: null,
  };
  ...
  const sortedAssetsWithValues = Array.isArray(
    ...
    ? [...assetFocusDataResult.assetsWithValues].sort(
  ...
  const handleAssetClick = useCallback(
    (assetDefinition: AssetDefinition) => {
      ...
      const matchingAsset = reduxState.assets.find(
        (asset: Asset) => asset.assetDefinitionId === assetDefinition.id
      );
      ...
      Logger.info(
        `AssetDashboardContainer: Opening modal for asset ${
          assetDefinition.name
        }, found matching transaction: ${!!matchingAsset}`
      );
    },
    [reduxState.assets]
  );
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      await executeAsyncOperation("refresh asset focus data", () =>
  ...
  // Provider-agnostische Batch-Methode für Intraday-Aktienpreise
  const updateBatchIntradayPrices = useCallback(async (definitions: AssetDefinition[]) => {
    const stockApiConfig = reduxState.isApiEnabled
      ? {
          apiKeys: reduxState.stockApiConfig?.apiKeys || {},
          selectedProvider:
            reduxState.stockApiConfig?.selectedProvider || "yahoo",
        }
      : { apiKeys: {}, selectedProvider: "yahoo" };

    return batchAssetUpdateService.updateBatchIntradayPrices(
      definitions,
      1,
      stockApiConfig
    );
  }, [reduxState.isApiEnabled, reduxState.stockApiConfig]);

  // Intraday history update handler
  const handleUpdateIntradayHistory = useCallback(async () => {
    Logger.info("Asset Focus intraday history update triggered");
    executeAsyncOperation("update intraday history", async () => {
      const stockDefinitions = reduxState.assetDefinitions.filter(
        (def: AssetDefinition) => def.type === "stock" && def.ticker
      );
      Logger.info(
        `Found ${stockDefinitions.length} stock definitions to update`
      );
      ...
      if (stockApiConfig.selectedProvider === "yahoo") {
        // Use batchAssetUpdateService for Yahoo (can handle both scenarios)
        Logger.infoService(
          "Asset Focus intraday history update triggered using main thread"
        );

        response = await updateBatchIntradayPrices(stockDefinitions);
      } else {
        // Use worker service for other providers (Finnhub, Polygon.io)
        Logger.infoService(
          "Asset Focus intraday history update triggered using worker"
        );

        response = await marketDataWorkerService.stockIntraday.updateBatch(
          stockDefinitions,
          1,
      ...
      if (
        ...
        const successfulResults = response.results.filter(
          (result) => result.success && result.updatedDefinition
        );

        for (const result of successfulResults) {
          const updatedDefinition = result.updatedDefinition!;
          const entriesCount =
            "entriesCount" in result ? result.entriesCount : 0;
          Logger.info(
            `Successfully updated intraday history for ${
              updatedDefinition.ticker
            }: ${entriesCount || 0} entries processed`
          );
          ...
          try {
            ...
            Logger.error(
              `Failed to dispatch update for ${updatedDefinition.ticker}: ${error}`
            );
          }
        }
        ...
        Logger.info(
          `Successfully processed intraday data for ${successfulResults.length} assets`
        );
      }
      // Handle batchAssetUpdateService response
      else if (response && "results" in response && response.results) {
        const successfulResults = response.results.filter(
          (result) => result.success && result.updatedDefinition
        );
        ...
        for (const result of successfulResults) {
          const updatedDefinition = result.updatedDefinition!;
          Logger.info(
            `Successfully updated intraday data for ${
              updatedDefinition.ticker || updatedDefinition.name
            }`
          );
    ...
    executeAsyncOperation,
    reduxState.assetDefinitions,
    reduxState.isApiEnabled,
    reduxState.stockApiConfig,
    dispatch,
